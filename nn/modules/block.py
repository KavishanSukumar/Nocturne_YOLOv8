...
class MaxSigmoidAttnBlock(nn.Module):
    ...
    def __init__(self, c1, c2, nh=1, ec=128, gc=512, scale=False):
        ...
        self.gl = nn.Linear(gc, nh * self.hc)
    ...
    def forward(self, x, guide):
        ...
        bs, _, h, w = x.shape
        guide = self.gl(guide)
        guide = guide.view(bs, self.nh, self.hc)
...
class C2fAttn(nn.Module):
    ...
    def forward(self, x, guide=torch.randn(1, 512)):
    ...
    def forward_split(self, x, guide=torch.randn(1, 512)):
...
# Create a algo to initialize guide

class Getguide():
    def __init__(self, gc, ec):
        """
        -Taking the input image
        -Applying the ehancing alogorithm
        -Change the image to tensor
        -If need resize(Optional)
        -Normalize pixel
        """
        return None
